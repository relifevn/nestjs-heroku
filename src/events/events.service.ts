...
export class EventsService {
	...
	async addGPSData(socket: Socket, gpsPostDto: GPSPostDto): Promise<void> {
		...
		const systemType =
			userSocket.deviceType === DEVICE_TYPE.FLAME_DETECTOR_ANDROID
	...
	async getActiveSockets(
		...
		const sockets = (await this.socketModel
			.find({ deviceType })
			.lean()) as ISocket[]
		const connectedSockets = await Promise.all(
			sockets.map(async cachedSocket => {
				const socket = await this.getSocketConnection(
					server,
					cachedSocket.socketId,
				)
				if (socket) {
					return cachedSocket
				}
